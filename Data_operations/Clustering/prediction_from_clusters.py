    ...
    with PdfPages(data_path + 'tendances_reabo_cluster_by_promo.pdf') as pdf:
        for i in range(8):
            ...
            for el in list_promo:
                ...
                df_filtered_delai_reabo['DATE_ACTE_REEL'] = pd.to_datetime(df_filtered_delai_reabo['DATE_ACTE_REEL'])
                df_filtered_delai_reabo['mois_annee'] = df_filtered_delai_reabo['DATE_ACTE_REEL'].dt.strftime('%Y-%m')
                df_filtered_delai_reabo = count_abo_conditions(df_filtered_delai_reabo, ["DELAI_REABO_WEEK_GROUP", 'DATE_ACTE_REEL'], 'ID_ABONNE')
                list_delais= df_filtered_delai_reabo["DELAI_REABO_WEEK_GROUP"].unique()
                plt.figure(figsize=(10, 6))
                for el_delais in list_delais:
                    sns.lineplot(x='DATE_ACTE_REEL', y='NB_ID_ABONNE', data=df_filtered_delai_reabo[df_filtered_delai_reabo["DELAI_REABO_WEEK_GROUP"] == el_delais].sort_values(by='DATE_ACTE_REEL'), label=el_delais)

                plt.title('Evolution of Reabo of the cluster ' + str(i) + " by time of reabo using the promo " + el)
                plt.xlabel('DATE_ACTE_REEL')
                plt.ylabel('NB_ID_ABONNE')
                plt.legend()
