...
def some_statistics_data_set(data_path):
    ...
    print(
        count_abo_conditions(
            df_Données_Reabos_odd, ["ID_ABONNE", "TYPE_PROMON"], "DATE_ACTE_REEL"
        )
    )
    print(
        count_abo_conditions(
            df_Données_Reabos_odd, ["ID_ABONNE", "DATE_ACTE_REEL"], "TYPE_PROMON"
        )
    )
...
def visualize_promos_data_set(
    data_path, df_name, list_promo=["Semaine genéreuse", "ODD 15 jours TC"]
):
    """
    ...
    for el in list_promo:
        sns.lineplot(
            x="DATE_ACTE_REEL",
            y="NB_ID_ABONNE",
            data=df[df["TYPE_PROMON"] == el].sort_values(by="DATE_ACTE_REEL"),
            label=el,
        )
