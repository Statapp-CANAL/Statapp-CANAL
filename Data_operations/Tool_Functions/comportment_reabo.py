...
def count_abo_conditions(df: pd.DataFrame,columns ,cond):
    ...
    df_filtered_groups = df_filtered_groups.sort_values(by = 'NB_' + cond, ascending=False) # Sort the DataFrame by the count column in descending order
...
def mean_empty_col(df,column,cond):
    ...
    series = df.groupby(df[column].isna()) # Group by whether the specified column is empty or not, then calculate the mean of the condition
